//Flatten nested array into a single array with the reduce and concat methods
let nestedArray = [[1,2,3],[4,5],[6,7,8]];

console.log(nestedArray.reduce((a,b) => a.concat(b)));
...
function highOrder(v,testFunc,updateFunc,bodyFunc) {
    for (let i = v; testFunc(i); i = updateFunc(i)) {
        bodyFunc(i);
    }
}

highOrder(0, test, update, body); 

//Everything

//Given an array and function, check to see if the function returns true for every instance of the array
function everything(array, func) {
...
//Same but with .some method
function someEverything(array, func) {
    return !array.some(v => !func(v)); //De Morgan's law a && b == !(!a || !b)
}

//.every only returns true if the given function returns true at index a && b && c exc. 
//.some returns true if the given function returns true at index a || b || c exc.
//Example array = [1,45,2,3]
//Example function(v) {if(v < 10) return true;}
//Process with someEverything(array,function):
    //function(1) -> true -> value negated to false
    //function(45) -> false -> value negated to true
    //function(2) -> true -> value negated to false
    //function(3) -> true -> value negated to false
    //array.some -> true -> value negated to false
        //Return false
...
//Process with someEverything(array,function):
    ...
    //function(2) -> true -> value negated to false
    //function(3) -> true -> value negated to false
    //array.some -> false -> value negated to true
        //Return true

//We know that .some will return true if all checks are true or if some checks are true. 
//.some will ONLY return false if every check is false. By negating the function return we make .some return false when all checks are true and negate that return it back to true.

//Dominant writing direction

//Script array taken from https://eloquentjavascript.net/code/scripts.js to use in this example.
...
var SCRIPTS = [
  ...
  function characterScript(code) { //Get unicode input
      for (let script of SCRIPTS) { //Check every script object within the SCRIPTS array
          if (script.ranges.some(([from, to]) => { //Access the ranges array within each script object //Use .some to check every range of numbers array within the range array. 
              return code >= from && code < to; //Return true if the unicode input lies within any of the ranges.
          } )) {
              return script; //Previous conditional returned true, so we return the current script we are on and exit the loop
          }
      }
...
function countBy(items, groupName) { //Get array input and a function
    ...
    for (let item of items) { //For each index in the input array
          ...
          if (known == -1) { //If the current name is not found in the 'counts' array
              counts.push({name, count: 1}) //add a new object to the 'counts' array including 'name' and 'count' properties
          } else {
...
function dominantDirection(text) { //Get an array of text
    let scripts = countBy(text, char => { //Checks every index in text array and returns a new array of objects with the direction value of each checked index
        let script = characterScript(char.codePointAt(0)); //Uses the characterScript to return what script each index(character) is written in
        return script ? script.direction : 'none'; //If the characterScript function returned an object, return that object's direction value
    }).filter(({name}) => name != 'none'); //Check every index in the array returned by the 'countBy' function and only return those with a name //Finnally store new array in 'scripts' variable
        //Remeber countBy returns in the following format [{name: , count: }]
    if (scripts.length == 0) return 'invalid entry'; //If the previously created array is empty, none of the input text is found in the SCRIPTS array
